...
(ert-deftest config-smoke/lazyvim-core-leader-bindings-exist ()
  (config-smoke--ensure-init-loaded)
  (dolist (state '(normal-state visual-state motion-state))
    (should (eq (config-smoke--leader-binding state "SPC SPC")
                #'editor-keys-find-files-root))
    (should (eq (config-smoke--leader-binding state "SPC ,")
                #'consult-buffer))
    (should (eq (config-smoke--leader-binding state "SPC /")
                #'consult-ripgrep))
    (should (eq (config-smoke--leader-binding state "SPC q q")
                #'save-buffers-kill-emacs))
    (should (eq (config-smoke--leader-binding state "SPC -")
                #'editor-windows-split-below))
    (should (eq (config-smoke--leader-binding state "SPC |")
                #'editor-windows-split-right))
    (should (eq (config-smoke--leader-binding state "SPC f e")
                #'tools-tree-toggle))
    (should (eq (config-smoke--leader-binding state "SPC x q")
